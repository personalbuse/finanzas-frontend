      ...
      <div className="space-y-8">
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 shadow-lg">
          <div className="flex items-center space-x-6 mb-8">
            <div className="h-20 w-20 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 flex items-center justify-center text-white text-3xl font-bold font-display shadow-lg">
              {currentUser.username?.charAt(0).toUpperCase()}
            </div>
            <div>
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white tracking-tight">{currentUser.username}</h2>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-6 border-t border-slate-100 dark:border-slate-700">
            <div className="p-4 bg-slate-50 dark:bg-slate-700/50 rounded-xl">
              <p className="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">{t('profile.memberSince')}</p>
              <p className="text-slate-900 dark:text-white font-bold mt-1 text-lg">
                {new Date(currentUser.created_at || Date.now()).toLocaleDateString()}
              </p>
            </div>
            <div className="p-4 bg-emerald-50 dark:bg-emerald-900/20 rounded-xl">
              <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-widest">{t('dashboard.balance')}</p>
              <p className="text-slate-900 dark:text-white font-bold mt-1 text-lg">
        ...
        <div className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl p-8 shadow-lg">
          <h3 className="text-lg font-bold text-slate-900 dark:text-white tracking-tight mb-6">{t('profile.settings')}</h3>
          
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 group hover:border-slate-200 dark:hover:border-slate-500 transition-all">
            ...
            <div 
              onClick={toggleTheme}
              className="flex items-center justify-between p-4 rounded-xl bg-slate-50 dark:bg-slate-700/50 border border-slate-100 dark:border-slate-600 group hover:border-slate-200 dark:hover:border-slate-500 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-slate-900 dark:bg-white flex items-center justify-center transition-colors">
                  {darkMode ? (
                ...
                <div>
                  <p className="text-sm font-bold text-slate-900 dark:text-white">{t('profile.darkMode')}</p>
                  <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{t('profile.darkModeDesc')}</p>
                </div>
              </div>
              <div className="px-4 py-2 rounded-lg bg-slate-100 dark:bg-slate-600 text-xs font-bold text-slate-600 dark:text-slate-300">
